...
def pre_data(data_X, k_data):  
    print('Start k-mer, and k =', k_data)
    obj = kmer_featurization(k_data)
    dataset = np.zeros((len(data_X), 4 ** k_data))
    for i in range(len(data_X)):
        kmer_feature = obj.obtain_kmer_feature_for_one_sequence(data_X[i], write_number_of_occurrences=True)  
        kmer_feature = kmer_feature / len(data_X[i])
        dataset[i] = kmer_feature
    print("dataset's shape after k-mer", dataset.shape)  
